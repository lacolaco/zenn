  ...
  const result = await syncNotionDatasource<ZennMetadata, ZennDatasource>({
    ...
    renderMarkdown: {
      ...
      generateFrontmatter: (baseFields, metadata) => {
        ...
        const publishedAtBase = metadata.createdAtOverride ? new Date(metadata.createdAtOverride) : metadata.date;

        return {
          title: baseFields.title,
          published_at: dayjs(publishedAtBase.toISOString()).tz('Asia/Tokyo').format('YYYY-MM-DD HH:mm'),
          topics,
